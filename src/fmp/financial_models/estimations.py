...
def estimate_da(
    ...
    da_est = pd.Series(index=pd.Index(years), name="depreciation_and_amortization")
    for year in years:
...
def estimate_change_in_wc(
    revenue: pd.Series, ratios: pd.DataFrame, past_wc: int, years: list[int]
) -> pd.Series:
    # Current assets
    short_term_investments_ratio_mean = ratios["short_term_investments_ratio"].mean()
    receivables_ratio_mean = ratios["receivables_ratio"].mean()
    inventory_ratio_mean = ratios["inventory_ratio"].mean()
    other_current_assets_ratio_mean = ratios["other_current_assets_ratio"].mean()
    # Current liabilities
...
def make_estimations(
    ...
    estimations["nopat"] = (
        estimations["ebitda"] - estimations["depreciation_and_amortization"]
    ) * (1 - tax_rate)
...
class CompanyEstimations(Company):
    def __init__(
        self, symbol: str, reports: pd.DataFrame, n_years: int, tax_rate: float = 0.27
    ):
        super().__init__(symbol, reports)
        self.n_years = n_years
        self.years = sorted(
            list(
                range(self.get_last_year() + 1, self.get_last_year() + self.n_years + 2)
            )
        )
