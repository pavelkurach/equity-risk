...
def estimate_ebitda(revenue: pd.Series,
                    ratios: pd.DataFrame,
                    years: list[int]) -> pd.Series:
...
def estimate_da(revenue: pd.Series,
                ratios: pd.DataFrame,
                years: list[int]) -> pd.Series:
...
def estimate_capex(revenue: pd.Series,
                   ratios: pd.DataFrame,
                   years: list[int]) -> pd.Series:
...
def estimate_change_in_wc(revenue: pd.Series,
                          ratios: pd.DataFrame,
                          past_wc: int,
                          years: list[int]) -> pd.Series:
    ...
    other_current_liabilities_ratio_mean = ratios[
        'other_current_liabilities_ratio'].mean()

    change_in_wc_est = pd.Series(index=pd.Index(years),
                                 name='change_in_wc')
    previous_wc = past_wc
    for year in years:
...
def make_estimations(reports: pd.DataFrame,
                     ratios: pd.DataFrame,
                     years: list[int],
                     tax_rate: float = 0.27) -> pd.DataFrame:
